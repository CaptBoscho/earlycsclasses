...
public:
	...
	void createAdjR()
	{
		...
		{
			vector<Pred> bod = Rules[j].getBody();
			for(unsigned int k=0; k<bod.size(); k++)
			{
				for(unsigned int l=0; l<Rules.size(); l++)
				{
					if(bod[k].getName() == Rules[l].getHead().getName())
					{
						string r1 = "R";
						r1 += to_string(j+1);
						string r2 = "R";
	...
	void createAdjQ()
	{
		for(unsigned int i=0; i<Querify.size(); i++)
		{
			for(unsigned int j=0; j<Rules.size(); j++)
			{
				if(Querify[i].getName() == Rules[j].getHead().getName())
				{
					string q = "Q";
					q += to_string(i+1);
					string r = "R";
	...
	void EvaluateRules(ofstream& out, vector<string> order)
	{
		out << "Rule Evaluation" << endl;
		int temp = 0;
		Rule ruler;
		Relation cousin;
		int bask = depend.Backside();
		do 
		{
			...
			{
				int k = depend.getNode(order[i]).getIndex();
				ruler = Rules[k];				
				Join(ruler, out);
	...
	void Printed(ofstream& out, Pred& p, int index)
	{
		...
		vector<string> order = depend.Numbers(out, index, Rules.size()); //P5
		EvaluateRules(out, order);
		//PrintFacts(out);
		
	}
	
	void Print2(ofstream& out, Relation& alter, Pred& p, int index)
	{
		//Printed(out, p, index);
